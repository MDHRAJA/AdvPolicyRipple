...
def interpret(prompt, objectives, size=500, rounds=20, seed=42):
    text = prompt.lower()
    ...
    config = SimulationConfig(population=PopulationConfig(preset=preset, size=size), policy_id=selected, policy_parameters={parameter_name: value}, rounds=rounds, seed=seed)
    return {
...
def recommend(config, objectives):
    ...
    best = candidates[0]
    return {'recommended': best, 'alternatives': candidates[1:3], 'boundary': 'Recommendations rank synthetic simulation outputs against user-selected objectives; they are not implementation advice or empirical forecasts.'}
