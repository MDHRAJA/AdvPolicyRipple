...
export function exportPresentationReport(input: ReportInput) {
  ...
  <style>
    @page { size: A4; margin: 15mm; } @page:first { margin: 0; }
    * { box-sizing: border-box; } body { color: #122235; font-family: Arial, Helvetica, sans-serif; font-size: 11px; line-height: 1.5; margin: 0; }
    ...
    section { border-top: 1px solid #c8d6e4; padding-top: 16px; margin-top: 18px; break-inside: avoid; } .cover { min-height: 297mm; display: flex; align-items: center; justify-content: center; padding: 36mm 26mm; background: radial-gradient(circle at 50% 25%, #155a63 0, #092039 44%, #061526 100%); color: #eff8ff; text-align: center; page-break-after: always; break-after: page; } .cover-inner { width: 100%; max-width: 145mm; } .cover h1 { font-size: 43px; letter-spacing: -1.8px; margin: 18px 0 16px; } .cover p { color: #c1d5e7; font-size: 14px; line-height: 1.65; max-width: 110mm; margin: 0 auto; }
    .brand { color: #56d6bc; font-weight: 800; letter-spacing: 1.5px; font-size: 13px; } .eyebrow { color: #267c6d; font-size: 10px; font-weight: 700; letter-spacing: 1.1px; margin-bottom: 6px; }
    .meta { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-top: 16px; } .meta div, .callout { background: #edf4f8; border: 1px solid #d4e1ea; border-radius: 7px; padding: 9px; } .cover .meta { margin-top: 28px; text-align:left; } .cover .meta div { background: rgba(255,255,255,.1); border-color: rgba(255,255,255,.22); } .cover .meta span { color:#b9d6e3; } .cover .meta b { color:#fff; } .meta span { display:block; color:#607286; font-size:9px; text-transform:uppercase; letter-spacing:.5px; } .meta b { display:block; margin-top:3px; font-size:12px; }
    ...
    <header class="cover"><div class="cover-inner"><div class="brand">POLICYFORGE</div><div class="eyebrow" style="color:#8ee7d5;margin-top:16px">CHENNAI POLICY DECISION SUPPORT</div><h1>Policy presentation report</h1><p>${isExploratory ? 'Census-informed synthetic Chennai scenario. Effects are modelled assumptions, not an observed forecast.' : 'Synthetic policy simulation for decision support, not a prediction of real people.'}</p>
      <div class="meta"><div><span>Experiment</span><b>${escapeHtml(policyTitle(config, result))}</b></div><div><span>Population</span><b>${config.population.size.toLocaleString()} synthetic agents</b></div><div><span>Generated</span><b>${new Date().toLocaleString()}</b></div></div></div>
    </header>
