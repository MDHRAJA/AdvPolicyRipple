...
function PlannerPageContent() {
  ...
  const [runningExploratoryScenario, setRunningExploratoryScenario] = useState(false);
  ...
  async function runExploratoryScenario() {
    setRunningExploratoryScenario(true);
    setError('');
    try {
      const scenario = await api.exploratoryScenario({ prompt, objectives: selected });
      if (typeof window !== 'undefined') {
        window.sessionStorage.setItem(
          'policyforge:lastSimulation',
          JSON.stringify({ config: scenario.config, result: scenario.result }),
        );
      }
      router.push('/results');
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'The exploratory AI scenario could not run.');
      setRunningExploratoryScenario(false);
    }
  }
  ...
  return <main className="page-shell planner-page">
    ...
    <div className="planner-grid">
      ...
      <section className="card p-6">
        ...
        {!plan ? triage ? <div className="planner-empty"><span>✦</span><h2>{triage.title}</h2><p>{triage.explanation}</p><div className="label planner-label">AI policy-design route</div><PolicyAdviceBrief advice={advice} error={adviceError} onRetry={() => void loadAdvice(prompt)} /><p className="helper">This scenario keeps AI assumptions separate from observed Census context and validated policy simulation.</p>{triage.mode === 'outside_catalog' ? <button className="btn primary planner-run" onClick={() => void runExploratoryScenario()} disabled={runningExploratoryScenario}>{runningExploratoryScenario ? 'BUILDING EXPLORATORY AI SCENARIO…' : 'SIMULATE EXPLORATORY AI SCENARIO →'}</button> : null}</div> : <div className="planner-empty"><span>✦</span><h2>Waiting for a policy question.</h2><p>Describe a local problem, choose priorities, and PolicyForge will decide whether it can create a simulation-ready proposal or needs a separate policy-design brief. To target a location, include a valid Chennai ward number—for example, “Chennai Ward 92”.</p></div> : <>
