    ...
    <div className="planner-grid">
      ...
      <section className="card p-6">
        ...
        {!plan ? <div className="planner-empty"><span>✦</span><h2>Waiting for a policy question.</h2><p>Describe a local problem, choose priorities, and PolicyForge will create a simulation-ready proposal.</p></div> : <>
          ...
          <section className="policy-note"><b>Expected simulated profile</b><span>Resource access {(plan.recommendation.recommended.preview.resource_access * 100).toFixed(1)}% · stress {(plan.recommendation.recommended.preview.stress * 100).toFixed(1)}% · trust {(plan.recommendation.recommended.preview.trust * 100).toFixed(1)}% · compliance {(plan.recommendation.recommended.preview.compliance * 100).toFixed(1)}%.</span></section><section className="policy-note"><b>How to apply the recommended option</b><span>{plan.recommendation.recommended.implementation.stages.map((step, index) => `${index + 1}. ${step}`).join(' ')}</span></section><button className="btn primary planner-run" onClick={apply}>REVIEW IN SIMULATOR →</button>
