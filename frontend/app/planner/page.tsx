    ...
    <div className="planner-grid">
      ...
      <section className="card p-6">
        ...
        {!plan ? <div className="planner-empty"><span>✦</span><h2>Waiting for a policy question.</h2><p>Describe a local problem, choose priorities, and PolicyForge will create a simulation-ready proposal. To target a location, include a valid Chennai ward number—for example, “Chennai Ward 92”.</p></div> : <>
          ...
          <p className="helper">{plan.interpretation}</p><p className="helper">Interpretation source: {plan.interpretation_source === 'openai' ? 'OpenAI-assisted, validated against PolicyForge policy limits' : 'Local rule-based fallback'}. Simulation metrics are always generated by PolicyForge.</p>
          <div className="plan-summary"><div><span>Population basis</span><b>{plan.policy_detail.population_basis}</b></div>{plan.proposed_config.target_wards?.length ? <div><span>Ward target</span><b>Ward {plan.proposed_config.target_wards.join(', ')}</b></div> : null}</div><div className="policy-note"><b>Proposed policy · {plan.matched_policy.name}</b><span>{plan.matched_policy.description} Parameter: {plan.policy_detail.parameter.replaceAll('_', ' ')} at {plan.policy_detail.value_percent}%.</span></div><div className="policy-note"><b>Why this was selected</b><span>Objectives: {plan.objectives.map((item) => item.replaceAll('_', ' ')).join(', ')}</span></div>
          <div className="policy-note"><b>Recommended option · {plan.recommendation.recommended.name}</b><span>{plan.recommendation.explanation}</span></div><div className="policy-note"><b>{plan.recommendation.recommended.policy_bundle.length > 1 ? 'Recommended policy combination' : 'Recommended percentage change'}</b><span>{plan.recommendation.recommended.policy_bundle.map((item) => item.name + ': ' + item.instruction).join(' ')}</span></div>
