...
const chennaiWards = Array.from({ length: 200 }, (_, index) => String(index + 1));
    ...
    <div className="workspace-grid">
      <section className="card p-6">
        ...
        <div className="space-y-4 mt-6">
          ...
          {preset === 'chennai_census_2011' && <div className="policy-note"><b>Ward targeting</b><span>{targetWards.length ? 'Policy effects will be directly applied to Wards ' + targetWards.join(', ') + '. Ward allocation is synthetic.' : 'All Chennai wards are selected: policy applies citywide across the synthetic Chennai sample.'}</span><div className="ward-target-actions"><button className="btn" onClick={() => setTargetWards([])}>Select all Chennai</button><button className="btn" onClick={() => setTargetWards([])} disabled={!targetWards.length}>Clear ward selection</button><button className="btn" onClick={() => router.push('/map')}>Choose on map →</button></div><label className="field"><span>Specific wards (optional)</span><select className="input ward-select" multiple value={targetWards} onChange={(event) => setTargetWards(Array.from(event.currentTarget.selectedOptions, (option) => option.value))}>{chennaiWards.map((ward) => <option key={ward} value={ward}>Ward {ward}</option>)}</select></label><span className="helper">Leave this list empty to target all Chennai wards. Use Ctrl/Cmd-click to select multiple wards.</span></div>}
